...
- **`format.ts`** — `BigNum` → **compound kanji** (chains up to 3 units, e.g. `123極4500載67正`) / english / scientific, plus `formatTime`. Within the万進法 kanji grid (`4 ≤ e < 72`, up to 無量大数) it's compound kanji; **beyond that it uses 華厳経「上数法」命数 by division** (`joUnits`: 矜羯羅 `10^112`, 阿伽羅 `10^224`, 最勝 `10^448`, 摩婆羅, 阿婆羅, 多婆羅, 界分 … each the previous *squared*): for `e ≥ 112`, find the largest命数 `U ≤ e`, show `formatNumber(value / 10^U.e)` + `U.name` — so **new kanji appears as you climb** (`1矜羯羅` → `100澗矜羯羅` → `1阿伽羅` → `1最勝` …). Because the units double, the quotient in a unit's upper range can itself exceed 無量大数, so 無量大数 may appear inside the coefficient (`1垓無量大数矜羯羅`). The gap `10^72`–`10^111` (no命数) uses **無量大数 stacked as a super-unit** (`compoundAboveMuryo`, `MURYO_E = 68`): `1万無量大数`, etc. Too large for命数/stacking (`k > MURYO_MAX_STACK = 16`, e.g. グラハム数級) **falls back to a recursive kanji power tower** (`kanjiOfNumber`) — `10^(1.7×10^308)` → `10^10^308`. `sci` compacts huge exponents via `formatExp`. Pure.
